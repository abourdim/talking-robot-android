#!/usr/bin/env bash
# ============================================================================
#  Talking Robot Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-06-talking-robot}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.talkingrobot}"
AVD_NAME="${AVD_NAME:-talking_robot_test}"
...
cmd_test() {
  ...
  local out="/tmp/talking-robot-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Talking Robot Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
